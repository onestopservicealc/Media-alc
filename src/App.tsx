  ...
  const vm = useMemo<AppVM>(() => {
    ...
    const popular: PopularVM[] = st.catalog.slice(0, 4).map(m => {
      ...
      return {
        ...
        stockText: stock.toLocaleString('en-US'),
        fbStyle: fbBox(m.category, 15),
        addLabel: selected ? '✓ อยู่ในคำขอแล้ว' : '＋ เพิ่มลงคำขอ',
        addBtn: {
          marginTop: 'auto', width: '100%', height: '44px', borderRadius: '10px', cursor: 'pointer',
          fontFamily: 'Kanit', fontWeight: 600, fontSize: '13.5px',
