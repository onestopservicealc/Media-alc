          ...
          <div key={m.id} style={m.cardStyle}>
            <div style={S('display:flex;align-items:flex-start;gap:12px')}>
              <div
                ...
                style={{ ...S('position:relative;width:64px;height:64px;border-radius:9px;overflow:hidden;background:#f1f2f4;flex-shrink:0'), cursor: m.imageUrl ? 'zoom-in' : 'default' }}
              >
            ...
            {m.outOfStock ? (
              <div style={S("margin-top:10px;height:44px;display:flex;align-items:center;justify-content:center;background:#f6f7f9;border:1px solid #e5e7eb;border-radius:10px;font-family:'Kanit';font-weight:600;font-size:13px;color:#9ca3af")}>หมดชั่วคราว</div>
            ) : m.notSelected ? (
              <button onClick={m.onAdd} style={S("margin-top:10px;width:100%;height:44px;background:#fff5f6;color:#e8112d;border:1px solid #fbd5da;border-radius:10px;font-family:'Kanit';font-weight:600;font-size:13.5px;cursor:pointer")}>＋ เพิ่มลงคำขอ</button>
            ) : (
              <div style={S('margin-top:10px;display:flex;align-items:center;gap:8px;background:#fff5f6;border:1px solid #fbd5da;border-radius:10px;padding:6px 8px')}>
